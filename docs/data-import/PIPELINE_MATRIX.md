...
| Saldos SIAFI / RAP | `Empenhos.tsx` | CSV | `parseSiafiCsv` + `syncSiafiDataToDb` | `empenhos` | Upload manual atualiza empenhos existentes e cria empenhos de exercicio ou RAP ausentes; aceita variacoes de pontuacao no cabecalho do SIAFI, detecta UTF-8, UTF-16LE, UTF-16BE e Latin-1; para RAP, a coluna `RESTOS A PAGAR NAO PROCES. LIQUIDADOS A PAGAR` alimenta `valor_liquidado_a_pagar` e e descontada de `saldo_rap_oficial`; quando a coluna nao vier, o valor considerado continua sendo `0`; o layout com virgulas e colunas `RESTOS A PAGAR ... (PROC E N PROC)` tem regressao em `siafiParser.test.ts` |
