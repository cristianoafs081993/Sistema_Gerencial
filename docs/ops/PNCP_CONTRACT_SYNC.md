...
## Pausa operacional do agendamento

A migration `20260909100000_pause_pncp_contract_sync_cron.sql` remove somente o
agendamento `sync-contratos-pncp-documentos-batch`. Use-a quando a chave
`pncp_sync_service_role_key` ainda não estiver no Vault e o job estiver gerando
falhas recorrentes. Ela não apaga documentos, instrumentos, histórico de
tentativas nem a Edge Function; atualizações autenticadas solicitadas pela tela
de detalhes do contrato permanecem disponíveis.

Para reativar o processamento em lote, cadastre primeiro a chave no Vault e
publique uma migration que recrie o mesmo job com intervalo de cinco minutos.
