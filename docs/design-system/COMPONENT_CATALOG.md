...
- `CreditoDisponivel`: pagina operacional que combina `FilterPanel`, `DataTablePanel` e `TablePagination` para leitura do saldo por PTRES/PI e importacao CSV no header.
...
- `SuapeCommandPalette` (extensao 1.9.22): central de comandos e busca flutuante global na extensão do SUAP (`command-palette.js` e `command-palette.css`) disparada por `Ctrl+K` ou `Cmd+K`. Focada no acesso rápido a **Empenhos** (com saldo em tempo real, credor, PI, processo e modal de detalhamento com métricas de execução) e **Contratos Ativos** (com consulta à API Comprasnet, valores, modal de detalhamento com histórico de aditivos e abas Fatura/Item com Fatura como padrão).
