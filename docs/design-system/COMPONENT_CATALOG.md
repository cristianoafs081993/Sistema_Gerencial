...
- `ComprasnetEtpAssistant` (extensao 1.9.19): botão adicional `br-button` na tela oficial de ETP e modal responsivo com iframe isolado. Usa `br-card`, `br-input`, alertas, estados de foco e tokens computados do próprio Comprasnet. O painel não oferece modo escuro próprio, não altera o `body` da página e nunca aciona `Concluir ETP`.
