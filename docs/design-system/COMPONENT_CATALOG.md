...
- `AtasRegistroPrecos`: pagina operacional que combina `FilterPanel`, `DataTablePanel`, `TablePagination`, badges de vigencia com contagem de dias restantes, filtro de atas vigentes, badges de vinculo/cobertura de itens com hover de participantes, drawer lateral com cards consolidados de execucao financeira, barras de saldo por item cruzadas com Comprasnet Contratos e links diretos ao PNCP.
...
- `ComprasnetEtpAssistant` (extensão 1.9.21): botão adicional `br-button` na tela oficial de ETP e modal responsivo com iframe isolado. Usa `br-card`, `br-input`, alertas, estados de foco e tokens computados do próprio Comprasnet. `Configurar minuta` reúne preferências gerais e checklist adaptado à seção aberta; a prévia completa marca as demais seções somente como referência e permite aplicar exclusivamente a seção atual. O painel não oferece modo escuro próprio, não altera o `body` da página e nunca aciona `Concluir ETP`.
