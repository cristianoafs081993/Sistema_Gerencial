...
- `SuapeClickHints` (extensão 1.9.52): modo global de dicas acionado por `Ctrl+;` ou `Cmd+;`. Exibe códigos mnemônicos determinísticos sobre os controles interativos visíveis da página, derivados prioritariamente do texto visível; siglas repetidas recebem sufixos numéricos na ordem visual. Enquanto o modo está aberto, a digitação filtra os pontos (mesmo com `Ctrl` mantido para preparar `Ctrl+Enter`), `Enter` abre o link na aba atual, `Ctrl+Enter`/`Cmd+Enter` abre links em uma nova aba via service worker da extensão (com resolução de elementos filhos, links relativos e fallback robusto), e `Esc` cancela, sem interferir na digitação normal fora do modo.
