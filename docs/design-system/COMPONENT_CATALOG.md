...
- `SuapeClickHints` (extensão 1.9.27): modo global de dicas acionado por `Ctrl+M` ou `Cmd+M`. Exibe códigos mnemônicos determinísticos sobre os controles interativos visíveis da página, derivados prioritariamente do texto visível; siglas repetidas recebem sufixos numéricos na ordem visual. Enquanto o modo está aberto, a digitação filtra os pontos, `Enter` confirma uma única ação e `Esc` cancela, sem interferir na digitação normal fora do modo.
