...
- `suapClickHints.test.ts` cobre a ativação por `Ctrl+;`, filtragem dos códigos, execução com `Enter`, foco de campos e abertura de links em nova aba com `Ctrl+Enter`; `suapExtensionPackage.test.ts` protege a ponte com o service worker.
