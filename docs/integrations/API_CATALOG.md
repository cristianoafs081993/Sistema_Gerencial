...
- Automação de upload de documentos externos no SUAP (extensao 1.9.50): adicionada a ação editável `suap_upload_document` na aba "Automações" do mapeamento de processos (`ProcessMappingDetailDrawer`) e pré-configurada na etapa de liquidação de bolsas (`DEFAULT_BOLSA_PROCESS_MAPPING`). Ao clicar no botão de check discreto (`.suape-flow-step-check`) na etapa correspondente no SUAP, a extensão armazena os parâmetros e abre a página de upload (`/processo_eletronico/documento_upload/{suapId}/`). O novo content script `upload-document.js` preenche automaticamente os três campos do formulário: Tipo de Conferência (`#id_tipo_conferencia` como "Cópia Simples" ou selecionado pelo usuário), Tipo de Documento (`#id_tipo` com sincronização nativa e Select2 v4 como "Liquidação" ou personalizado) e Assunto (`#id_assunto`). Todos os campos e opções são 100% editáveis pelo usuário no editor de mapeamentos, com persistência direta e suporte a variáveis dinâmicas.
