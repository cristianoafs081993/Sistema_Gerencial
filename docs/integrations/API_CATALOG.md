...
- Persistencia da sessao (extensao 1.9.30): o service worker e o unico responsavel por armazenar e renovar o `refresh_token`, com serializacao para impedir renovacoes concorrentes. Popup e content scripts recebem somente um `accessToken` efemero com `expiresAt`; o refresh token nunca atravessa mensagens ou iframes. Falhas de rede/5xx preservam a sessao para nova tentativa; revogacao, token ja usado ou expiracao confirmada pelo Supabase encerram a sessao local com aviso explicito.
- Mapeamento de processo e fluxo BPMN (extensao 1.9.35 e 1.9.36): `buildContext` inclui os dados extraídos diretamente do DOM da página do SUAP (`assunto`, `beneficiario`, `cpfCnpj`, `caixa` e `route.assunto`) no payload enviado ao iframe `/suap-extensao/processo-info`. O iframe emite o snapshot inicial e seleciona o mapeamento correspondente (ex.: Bolsas vs Nota Fiscal) imediatamente no primeiro frame, autenticando com a sessão efêmera para carregar os mapeamentos publicados atualizados sem reverter indevidamente para o padrão de nota fiscal. Na versão 1.9.36, os títulos das etapas do fluxo padrão de Bolsas foram alinhados com o diagrama BPMN conciso do SIAGES, e o serviço `processMappingsService` passou a comparar timestamps (`updatedAt`) para impedir que dados obsoletos em cache sobrescrevam definições mais recentes.
- Interface do painel e seções colapsáveis (extensao 1.9.37): os botões de minimizar (−/+) e maximizar (⛶) foram alinhados horizontalmente na mesma linha do título "SIAGES" no cabeçalho (`.suape-brand-row`). As seções de dados do processo ("Processo", "Beneficiário", "Documento e pagamento", "Dados bancários", "Retenções e empenhos") e o card "Caminho do processo" passaram a ser colapsáveis interativamente, com chevron indicador de estado (`▾` / `▸`) e persistência no storage local (`siages-toolkit-sections-collapsed`) para manter a preferência do usuário entre atualizações e re-renderizações de snapshot.
- Ajuste manual da etapa atual do processo (extensao 1.9.38): o usuário pode clicar diretamente em qualquer etapa da lista de "Caminho do processo" para defini-la como a etapa atual (`status: 'current'`), corrigindo eventuais falhas de identificação automática pelos trâmites do SUAP. As etapas anteriores passam para "Concluída" e a seguinte para "Próxima etapa". O ajuste é persistido por processo e mapeamento (`siages-process-step:{suapId}:{mappingId}`). A seleção ocorre de forma limpa e direta sem barras de aviso intrusivas, e clicar novamente sobre a etapa selecionada restaura a detecção automática a qualquer momento.
- Automação de etapas e botão de check discreto (extensao 1.9.39): na gaveta de detalhes de etapas do mapeamento de processos (`ProcessMappingDetailDrawer`), foi adicionada a aba "Automações" após a aba "Checklist". Nela, o usuário pode habilitar e configurar ações acionadas pelo botão de check discreto (`.suape-flow-step-check`) exibido na etapa atual do "Caminho do processo" no SUAP. As ações configuráveis incluem avanço direto de etapa (`advance_step`), abertura de links/sistemas externos com preenchimento de variáveis dinâmicas (`open_url`), cópia de minutas e despachos com dados do processo (`copy_text`), criação direcionada de documentos no SUAP (`suap_document`) e disparo de webhooks HTTP (`custom_webhook`). As tags `{processNumber}`, `{suapId}`, `{beneficiario}`, `{cpfCnpj}`, `{assunto}`, `{valor}`, `{etapa}`, `{contrato}` e `{empenho}` são resolvidas em tempo de execução, e um toast transitório (`.suape-automation-toast`) fornece feedback visual da conclusão.
- Alerta de pré-doc no SIAFI (extensao 1.9.40): o novo content script `siafi-predoc-alert.js` acompanha cada linha da edição de DH e bloqueia, com diálogo próprio, abas internas, links, cancelamento e `Registrar Alterações` quando houver qualquer favorecido sem pré-doc. `Verificar Consistência` permanece liberado, e saídas do navegador usam `beforeunload`. O alerta não grava dados, não chama endpoints e não altera a confirmação manual da liquidação.
- Correção do fluxo de edição do pré-doc no SIAFI (extensao 1.9.41): os controles AJAX `Confirmar` e `Descartar` da lista de favorecidos ficam liberados para que o usuário conclua a inclusão/alteração e habilite o botão `Pré-Doc`; a proteção passa a reconhecer também `Registrar` e `Salvar Rascunho`, sem bloquear submits internos não relacionados à saída da página.
- Correção visual da paleta da extensão (extensao 1.9.42): o modal de visualização de empenhos força o grid de métricas a ocupar toda a largura disponível e usa colunas responsivas, evitando que estilos globais do SUAP estreitem e empilhem os cards.
- Restrição do alerta de pré-doc à aba correta (extensao 1.9.43): `siafi-predoc-alert.js` só bloqueia saída, registro e `beforeunload` quando a aba `Dados de Pagamento` está ativa; as demais abas da edição de DH não exibem o aviso.
- Cobertura de pré-docs de deduções (extensao 1.9.46): na aba `Dados de Pagamento`, o alerta também conta linhas de dedução sem pré-doc e mantém liberados os controles internos `Confirmar`/`Descartar` necessários para concluir o preenchimento dessa lista.
- Alerta de pré-doc no Comprasnet (extensao 1.9.49): o content script `comprasnet-predoc-alert.js` acompanha favorecidos e deduções na rota de alteração de apropriação. Ele identifica o pré-doc preenchido pela presença do controle de remoção que o Comprasnet acrescenta à célula, bloqueia troca de abas, `Confirmar Dados de Pagamento`, `Apropriar SIAFI`, links e saída do navegador quando a aba `Dados de pagamento` está ativa, sem bloquear o botão que abre o editor do pré-doc.
- Automação de upload de documentos externos no SUAP (extensao 1.9.50 / 1.9.51): adicionada a ação editável `suap_upload_document` na aba "Automações" do mapeamento de processos (`ProcessMappingDetailDrawer`) e pré-configurada nas etapas de instrução e liquidação de bolsas (`DEFAULT_BOLSA_PROCESS_MAPPING`, cobrindo etapas 1 e 4). Ao clicar no botão de check discreto (`.suape-flow-step-check`) na etapa correspondente no SUAP, a extensão delega a abertura de aba ao background service worker (`chrome.tabs.create` imune a bloqueador de popups do Chrome) e armazena os parâmetros com fallback triplo (URL hash `#siagesUpload=`, `chrome.storage.local` e `sessionStorage`). O content script `upload-document.js` preenche automaticamente os três campos: Tipo de Conferência (`#id_tipo_conferencia` como "Cópia Simples" ou selecionado pelo usuário), Tipo de Documento (`#id_tipo` com sincronização nativa e Select2 v3/v4 como "Liquidação" ou personalizado) e Assunto (`#id_assunto`). Todos os campos e opções são 100% editáveis no editor de mapeamentos, sincronizados em tempo real do portal SIAGES para a extensão via `siages-portal-sync.js` (`chrome.storage.local`) e injetados de forma transparente no iframe bridge e no fluxo do SUAP.
...
- Auditoria Semântica Rigorosa via Gemini (com fallback entre `gemini-2.5-flash`, `gemini-2.5-flash-lite` e `gemini-1.5-flash`):
  - Auditoria em Lote (Batch Auditing): todos os candidatos coletados para um item são submetidos e auditados conjuntamente em uma única chamada de IA (`auditCandidatesBatchWithGemini`), reduzindo em mais de 75% as chamadas externas, imunizando o sistema contra limites de RPM/rate-limiting e acelerando a resposta multi-item.
