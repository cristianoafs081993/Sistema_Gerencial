...
- na Requisicao de Compra, o bloqueio usa o saldo local/SIAFI do empenho; o saldo por item exibido e contratual e informativo, calculado apenas com vinculos oficiais materializados em `contratos_api_fatura_itens`
...
- Para apoiar a analise do processo sem troca de aba, o content script tambem cria um card financeiro no HTML do SUAP e um iframe oculto em `/suap-extensao/processo-info`. Esse iframe anuncia `siages:suap-process-info-ready`, recebe o mesmo contexto validado, consulta `processos` por `suap_id` sob RLS e monta o resumo com `empenhos`, `contratos`, `contratos_empenhos`, `contratos_api`, `contratos_api_empenhos` e cache local `contratos_api_empenho_liquidacoes_cache`. A leitura e local/cache: nao cria banco, Edge Function nova nem refresh automatico de APIs externas ao abrir a pagina. Se houver contrato no processo, filtra pelo contrato; se nao houver beneficiario identificado, nao exibe card. O payload do card expõe apenas empenhado, saldo e liquidações detalhadas por empenho, sem campos de pagamento, e a extensão prioriza inserir o card no fim da coluna direita da timeline de tramitação do SUAP (`Recebido por`, `Encaminhado por`, `Registro de ações`), com conteúdo responsivo para não sobrepor documentos ou cards vizinhos.
