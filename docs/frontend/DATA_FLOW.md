...
- O Dashboard conta com um seletor Radio com opções **"Avaliações"** (apresentando Satisfação Média, Total de Ocorrências, Taxa e Tempo Médio de Resolução, Gráfico de Avaliações, Pareto de Falhas e Salas com Atenção Prioritária) e **"Insumos"** (apresentando **Total de Requisições**, **Valor Total Gasto** em R$ com quantitativo consolidado de itens consumidos no período, gráfico em rosca de **Distribuição por Categoria** com participação percentual e monetária por grupo de alimentos/insumos, **Evolução Temporal do Valor Gasto** com insumos, **Consumo Geral de Insumos** em barras horizontais com sanitização automática de descrições longas/códigos CATMAT e alternância Top 8/Todos, e **Top 5 Ambientes**, com filtragem dedicada e dinâmica por Tipo de Insumo).
- O detalhamento analítico de **Consumo de Insumos** funciona como um recurso de **Detalhamento interativo**, acionado pelo botão **Detalhar** no gráfico `Distribuição por Categoria` e por clique nos gráficos `Evolução Temporal de Insumos`, `Consumo Geral de Insumos` e `Top 5 Ambientes`, abrindo modal completo com agrupamentos por ambiente/dia, busca instantânea, colunas de quantidade física e valor total financeiro em R$, e filtros por data/material.
