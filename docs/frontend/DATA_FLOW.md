...
- o botão opt-in `Extrair com PDFs individuais — piloto` inventaria os anexos da página do processo, inclui por segurança os títulos não reconhecidos e ignora apenas IMR, certidões/documentação complementar, conta vinculada, relatório de recebimento provisório e folhas de pagamento; até quatro PDFs elegíveis são baixados em paralelo. A geração do PDF completo começa ao mesmo tempo, continua canônica para o Editor de Documentos e é usada como fallback obrigatório se qualquer peça relevante falhar.
