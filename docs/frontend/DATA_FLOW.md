...
- o grafico de gastos por contrato exibe linhas mensais acumuladas por contrato, identifica a serie por fornecedor e numero do contrato, seleciona automaticamente apenas 1 contrato (de forma aleatoria entre aqueles com gasto no periodo) no conjunto carregado e permite selecao multipla manual sem alterar a aba RAP
...
- a aba tambem exibe bullet chart de projecao anual por contrato selecionado: a base comparativa exibe o `Saldo` real dos empenhos vinculados em `saldoEmpenhos` (calculado com a mesma lógica unificada de `Contratos.tsx` que soma empenhos locais e da API preferindo o saldo local se houver vínculo ou número correspondente), `Liquidado` soma faturas executadas no periodo e `Projetado` anualiza a media mensal observada frente a essa base de saldo (sem nunca ultrapassar o valor total do contrato)
- o bullet chart apresenta os valores na ordem `Saldo`, `Liquidado` e `Projetado`, com hover de rastreabilidade contendo liquidações consideradas, empenhos vinculados, saldo dos empenhos e meses usados na projecao. Na base do gráfico, exibe a "Provável necessidade de empenho" (projetado - (liquidado + saldo dos empenhos)) e a "Cobertura provável até" (mês estimado em que o saldo seria suficiente no ritmo atual de gastos)
- a aba apresenta também um painel de **Heatmap de Cobertura de Empenhos** para todos os contratos ativos. Este heatmap mapeia a razão `(Liquidado + Saldo dos empenhos) / Projetado`, classificando os contratos em três níveis críticos visuais (Crítico < 70%, Atenção 70%-99%, Adequado >= 100%) em um grid interativo com HoverCards contendo o detalhamento financeiro individual e as projeções anuais de cobertura.
