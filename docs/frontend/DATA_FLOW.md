...
- a mesma ponte `/suap-extensao/processo-info` publica `siages:suap-process-flow`: `process-document.js` extrai os tramites visiveis da timeline, envia a sequencia com o `suap_id` e o iframe resolve o mapa BPMN publicado em `process_mappings`. O painel compacto mostra a etapa atual, a proxima etapa, evidencias do SUAP e o link para `/mapeamentos/:mappingId`; a selecao manual do mapa fica em `chrome.storage.local` por processo. Se a tabela ainda nao tiver mapas publicados, a extensao usa o mapa operacional local de fallback e continua funcional.
...
- **Pesquisa de Preços Conversacional & Card Interativo**: respostas com `priceResearchResult` renderizam `PriceResearchChatCard.tsx`, com mapa de preços, auditoria semântica dos editais/TRs no PNCP, cálculo da mediana e CV (IN 65/2021), botões de exportação (PDF, SUAP e Excel), badges com os sinônimos oficiais utilizados na busca e alerta visual para itens com amostra inferior a 3 cotações válidas
- **Base Local de Preços de Referência & Busca Híbrida**: a pesquisa conversacional consulta prioritariamente a base própria em `preco_referencia_itens` via RPC `match_preco_referencia_hibrido` (usando embeddings do Gemini `gemini-embedding-001`, similaridade de cosseno a 50%, FTS em português a 30% e trigramas a 20%), recorrendo a sinônimos locais e à busca externa no PNCP apenas quando o banco local retornar menos de 3 cotações válidas; a base é mantida sincronizada via Edge Function `sync-precos-referencia`, job diário `pg_cron` e service frontend [precoReferencia.ts](/C:/Users/crist/OneDrive/Desktop/Obsidian/01%20-%20Projetos/Apps/Sistema_Gerencial/src/services/precoReferencia.ts)
...
- Mesmo que o usuário possua associações antigas a outros grupos, um terceirizado de refeitório recebe somente as telas `/requisicao-compra` e `/contratos`; o login SUAP também remove memberships incompatíveis.
...
- O módulo opera com **3 status principais**: `draft` (Rascunho), `enviada_fornecedor` (Enviada ao Fornecedor) e `liquidada` (Liquidada).
...
- Ao mudar para `liquidada`, o desconto temporário deste módulo deixa de existir, considerando que a liquidação oficial registrada no SIAFI já reduz diretamente o saldo do empenho (evitando duplicidade de desconto).
...
Desde a versao 1.9.20, o login da extensao fica persistido em `chrome.storage.local` ate o usuario acionar `Sair`. O service worker centraliza e serializa a renovacao do `refresh_token`; falhas temporarias nao removem a sessao, e popup/content scripts nao renovam tokens diretamente.
