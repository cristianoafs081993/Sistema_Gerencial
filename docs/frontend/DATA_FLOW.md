...
- a tabela tambem permite gerar o `Despacho de Liquidacao` por processo ou em lote, sem sair de `/suap`; a fila da geracao fica em `sessionStorage` para sobreviver a atualizacoes acidentais da pagina
...
- cada item da fila pode ser editado, copiado e clonado individualmente no SUAP. A clonagem usa o mesmo modo de revisao ou de salvamento apos confirmacao do gerador de documentos e nao abre varias abas automaticamente em lote
...
- o clone manual de despacho no SUAP usa `suapCloneAutomation.ts` para abrir o modelo `1026154` com assunto e HTML do documento no fragmento da URL; a extensao `suap-atividades-extension` roda dentro de `suap.ifrn.edu.br`, preenche o campo `assunto`, so clica `Salvar` quando o usuario confirmou essa acao no SIAGES e, apos a tela `visualizar_documento/{id}`, tenta abrir `Editar > Texto` para preencher o corpo no editor Tiny
