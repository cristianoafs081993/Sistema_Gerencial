...
- Ao criar ou editar uma requisicao, o empenho e a referencia principal: a selecao de contrato e opcional e nao condiciona o carregamento dos itens. A tela consulta `transparenciaService.getItensEmpenhoPortal(..., { includeHistorico: true })` para montar os itens a partir dos subitens da NE, usando `valorAtual`/historico do Portal da Transparencia e o saldo global do empenho calculado pela mesma regra de `/empenhos`. O cache desses subitens e pre-aquecido diariamente no Supabase para empenhos vinculados a terceirizados e requisicoes recentes em rascunho/revisao; a chamada sob demanda permanece como fallback.
- O saldo por item em `/requisicao-compra` parte do item da NE, abate liquidacoes descobertas por empenho na API publica/cache de contratos quando o subelemento permite associacao, e abate requisicoes em `review` para o mesmo `source_item_key`. A origem fica persistida em `requisicao_compra_itens.source_*`; itens manuais continuam permitidos quando a API/cache nao retorna subitens.
