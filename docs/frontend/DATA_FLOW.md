...
- a mesma ponte `/suap-extensao/processo-info` publica `siages:suap-process-flow`: `process-document.js` extrai os tramites visiveis da timeline, envia a sequencia com o `suap_id` e o iframe resolve os mapas BPMN publicados em `process_mappings`, mesclados aos mapas operacionais locais de contingência. O painel compacto mostra a etapa atual, a proxima etapa, evidencias do SUAP e o link para `/mapeamentos/:mappingId`; a selecao manual do mapa fica em `chrome.storage.local` por processo e sempre prevalece. Sem uma selecao manual, assuntos que contenham `bolsa` ou `bolsista` usam automaticamente o mapa de liquidação e pagamento de bolsas; os demais processos continuam no mapa de nota fiscal.
