...
- o mesmo modal consulta `transparenciaService.getItensEmpenhoPortal` para ler `portal_transparencia_empenho_itens_cache*` e exibir somente descricao e subelemento dos subitens. Quando o cache esta ausente ou vencido, o service chama a Edge Function `refresh-portal-transparencia-itens-cache`, que consulta `/despesas/itens-de-empenho` no Portal da Transparencia pelo servidor e salva as linhas no Supabase; a consulta direta pelo browser fica apenas como compatibilidade quando a migration/function ainda nao existem. O service ja retorna `valorAtual` e historico do item para eventual uso futuro, mas a UI nao mostra saldo/valor dessa API porque a base pode estar defasada.
...
- o bullet chart de projeção anual por contrato selecionado exibe o `Saldo` real dos empenhos vinculados em `saldoEmpenhos` (calculado com a mesma lógica unificada de `Contratos.tsx` que soma empenhos locais e da API preferindo o saldo local se houver vínculo ou número correspondente), `Liquidado` soma faturas executadas no período e `Projetado` anualiza a média mensal observada frente a essa base de saldo (sem nunca ultrapassar o valor total do contrato).
- o bullet chart apresenta os valores na ordem `Saldo`, `Liquidado` e `Projetado`, com hover de rastreabilidade contendo liquidações consideradas, empenhos vinculados, saldo dos empenhos e meses usados na projeção. Na base do gráfico, exibe a "Provável necessidade de empenho" (projetado - (liquidado + saldo dos empenhos)) e a "Cobertura provável até" (mês estimado em que o saldo seria suficiente no ritmo atual de gastos).
...
- na página SUAP `plan_estrategico/plano_concluido/8/`, a extensão injeta o resumo por dimensão sem consultar o Supabase diretamente. O content script abre o iframe protegido `/suap-extensao/plano-resumo`, valida `origin`, `source`, `version`, `planId = 8`, URL e payload antes de enviar ou aceitar mensagens. A rota leve usa a sessão autenticada do SIAGES e consulta apenas `atividades`, `descentralizacoes` e `empenhos`; as RLS dessas consultas preservam o isolamento por `org_id`.
...
- Ao criar ou editar uma requisicao, os empenhos sao selecionados em um combobox pesquisavel com multipla selecao. A selecao de contrato e opcional e apenas filtra os empenhos permitidos quando ha vinculo em `contratos_empenhos`; para terceirizados, a lista continua limitada a permissoes diretas por `terceirizado_permissions.empenho_id`.
- Cada NE selecionada cria um grupo proprio de itens em `/requisicao-compra`. A tela consulta `transparenciaService.getItensEmpenhoPortal(..., { includeHistorico: true })` para cada NE e monta os itens a partir dos subitens da respectiva NE, usando `valorAtual`/historico do Portal da Transparencia e o saldo global do empenho calculado pela mesma regra de `/empenhos`. O cache desses subitens e pre-aquecido diariamente no Supabase para empenhos vinculados a terceirizados e requisicoes recentes em rascunho/revisao; a chamada sob demanda permanece como fallback.
- O saldo por item em `/requisicao-compra` parte do item da NE, abate liquidacoes descobertas por empenho na API publica/cache de contratos quando o subelemento permite associacao, e abate requisicoes em `review` para o mesmo `source_item_key` e mesmo `empenho_id`. A origem fica persistida em `requisicao_compra_itens.source_*`; a NE do item fica em `requisicao_compra_itens.empenho_id/empenho_numero`; itens manuais continuam permitidos quando a API/cache nao retorna subitens.
...
- O formulário mostra saldo e total separadamente por NE selecionada e bloqueia o envio para revisão quando qualquer grupo excede o saldo da respectiva NE ou quando algum item nao possui `empenhoId`. O salvamento usa a RPC transacional `save_requisicao_compra`, que grava `requisicao_compra_empenhos`, mantem `requisicoes_compra.empenho_id/empenho_numero` preenchidos com a primeira NE por compatibilidade, repete a validacao no banco e desconta requisicoes que ja estejam em revisao por NE.
