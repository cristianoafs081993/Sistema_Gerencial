...
- no [Dashboard.tsx](/C:/Users/crist/OneDrive/Desktop/Obsidian/01%20-%20Projetos/Apps/Sistema_Gerencial/src/pages/Dashboard.tsx), a tabela "Detalhamento por Origem" da aba Exercício Corrente permite drill-down via `DashboardOrigemAtividadesModal.tsx`, exibindo as atividades planejadas daquela origem com o saldo oficial `saldo_disponivel` do Plano 8 do SUAP (`saldo > 0`), correlacionando empenhos SIAFI às atividades por vínculos diretos, processos, siglas e termos-chave via `atividadeEmpenhoMatching.ts`, com busca textual e abertura de `AtividadeDialog.tsx` para edição direta sincronizada com `refreshData`.
- no [Dashboard.tsx](/C:/Users/crist/OneDrive/Desktop/Obsidian/01%20-%20Projetos/Apps/Sistema_Gerencial/src/pages/Dashboard.tsx), origens de recursos externos (como `230446` - PNAE/recurso externo) são excluídas especificamente do cálculo do card **Descentralizado** e das métricas de descentralização (`EXCLUDED_DESCENTRALIZACAO_ORIGENS`), preservando a consistência do crédito orçamentário planejado do campus, enquanto permanecem visíveis com seus respectivos empenhos na tabela "Detalhamento por Origem" e no restante do sistema.
- no [Dashboard.tsx](/C:/Users/crist/OneDrive/Desktop/Obsidian/01%20-%20Projetos/Apps/Sistema_Gerencial/src/pages/Dashboard.tsx), o card **Liquidado/Descentralizado** resolve o numerador por empenho tentando primeiro as liquidações vinculadas de `documentos_habeis`; sem esse detalhe, preserva o valor oficial de `empenhos`, usa a API pública de contratos como fallback e, por fim, o valor legado. O `refreshData` também invalida essas consultas auxiliares após importações para que o gauge e a evolução mensal reflitam a mesma atualização.
- no [Dashboard.tsx](/C:/Users/crist/OneDrive/Desktop/Obsidian/01%20-%20Projetos/Apps/Sistema_Gerencial/src/pages/Dashboard.tsx), o painel de filtros lateral (`DashboardFiltersSheet.tsx`) disponibiliza como primeiro critério a **Origem de Recurso**, seguido pelo **Plano Interno (PI)** (em substituição ao filtro de dimensão) e pelo intervalo de datas. O filtro de Plano Interno aplica correspondência flexível via `planoInternoFilters.ts`, cobrindo códigos SIAFI de 11 caracteres, descrições compostas do SUAP e variações alfanuméricas em atividades planejadas, empenhos e descentralizações.
...
- nas mesmas paginas de processo, a extensao tambem injeta um card SIAGES de empenhos do beneficiario e usa um iframe oculto em `/suap-extensao/processo-info`. O JWT autenticado no popup da extensao e entregue ao iframe somente por `postMessage` validado para `https://suap.ifrn.edu.br`; assim, o resumo nao depende de cookies de terceiros no iframe. A sessao privada fica em `chrome.storage.local` e o service worker a renova em segundo plano antes do vencimento; falhas de rede ou de renovacao, inclusive revogacao detectada pelo servidor, nao removem a sessao automaticamente: ela so e removida quando o usuario aciona `Sair`. Respostas 400/401 do endpoint de senha sao tratadas como credenciais invalidas; se o content script tiver sido invalidado por uma atualizacao da extensao, a tela orienta recarregar a pagina do SUAP (e o popup pode ser reaberto). O iframe desativa sua renovacao propria para nao invalidar o refresh token guardado pela extensao. A rota usa a sessao efemera para consultar o processo sob RLS e devolve por `postMessage` o resumo de empenhado, saldo e liquidacoes em cache detalhadas por empenho. Quando o processo tem contrato identificado, o resumo fica restrito aos empenhos daquele contrato; sem beneficiario identificado, o card nao e exibido. O card e o botao `Gerar documento` compartilham um painel fixo no canto inferior direito, para nao depender da estrutura ou da coluna da timeline do SUAP. A conciliacao das liquidacoes usa as chaves resumida e longa do empenho, evitando perder linhas quando o cache armazena o prefixo da UG/gestao
- a mesma ponte `/suap-extensao/processo-info` publica `siages:suap-process-flow`: `process-document.js` extrai os tramites visiveis da timeline, envia a sequencia com o `suap_id` e o iframe resolve os mapas BPMN publicados em `process_mappings`, mesclados com prioridade às edições operacionais locais do usuário (`siages_process_mappings_v2`) e mapas padrão de contingência. As edições realizadas em `/mapeamentos` são persistidas localmente no `localStorage` e sincronizadas no Supabase via `processMappingsService.saveMapping`/`saveAll`. O fluxo é emitido imediatamente após a recepção do contexto na ponte e revalidado na conclusão da carga do processo. O painel compacto da extensão exibe o rótulo "Mapeamento aplicado" e o seletor na mesma linha horizontal (`suape-flow-chooser`), além da etapa atual, da próxima etapa, de evidências do SUAP e do link para `/mapeamentos/:mappingId`; a seleção manual do mapa fica em `chrome.storage.local` por processo e sempre prevalece. Sem seleção manual, assuntos que contenham `bolsa` ou `bolsista` usam automaticamente o mapa de liquidação e pagamento de bolsas; os demais processos continuam no mapa de nota fiscal.
- a origem publica do SIAGES e configurada no popup da extensao; o iframe pode autenticar no proprio modal quando nao houver sessao, e `Clonar no SUAP` continua abrindo uma nova aba com a automacao de assunto, injecao de conteudo no TinyMCE (no contexto principal da pagina), atualizacao de textareas e sincronizacao protegida antes do salvamento
- processos com extracao `success` ou `incomplete_extraction` alimentam o despacho com o contexto extraido pela IA; a lateral permite trocar o modelo da minuta, como servico, aquisicao, projeto, bolsa sem projeto e auxilios, regenerando o HTML com os dados ja extraidos antes de copiar ou clonar. Servico preserva o texto de prestacao, enquanto aquisicao usa o ateste de recebimento do objeto adquirido. Na extracao parcial, os campos ausentes ficam marcados no texto para revisao. Processos sem IA concluida abrem somente os campos manuais pendentes, ja preenchidos com os dados basicos do processo
- cada item da fila pode ser editado, copiado e clonado individualmente no SUAP. A clonagem abre diretamente em modo de revisao para que o usuario possa validar o assunto e o texto no TinyMCE antes de salvar no SUAP.
...
- **Pesquisa de preços conversacional**: `PriceResearchChatCard` apresenta uma pesquisa preliminar, requisitos atendidos/ausentes e evidência documental apenas quando o PDF foi processado. O salvamento produz rascunho; exportações exigem revisão humana.
- **Base local de preços**: `match_preco_referencia_v2` combina rankings por RRF, identifica o modelo de embedding e filtra dados por item. A ingestão usa cursor transacional e autenticação. Consulte [precisão e implantação](../ops/PRECOS_PRECISAO.md).
- **Diálogo de Esclarecimento Técnico**: quando a descrição da demanda for preliminar ou vaga, o agente retorna perguntas objetivas e botões de sugestões rápidas no chat; ao responder ou clicar na sugestão, o contexto anterior é unificado contextualmente e a cotação prossegue sem perda de dados
- **GovFlow Core SDK & Conciliação Orçamentária/Contratual**: o sistema conta com a biblioteca em `src/sdk/` com módulos de Orçamento, Empenhos, Descentralizações, Contratos e Conciliação. Na Edge Function `assistente-gerencial`, quando a pergunta cita uma Origem de Recurso/PTRES (ex: `231798`), o backend consulta diretamente o conjunto completo de atividades do SUAP, notas de crédito do SIAFI e notas de empenho daquele PTRES. O algoritmo de conciliação trilateral separa o saldo do planejamento no SUAP (`Planejado - Empenhado`) do saldo financeiro real no SIAFI (`Descentralizado - Empenhado`), identificando a nível de Plano Interno (PI) quais empenhos foram emitidos sem previsão de atividade correspondente no Plano 8 e munindo a IA com o diagnóstico contábil exato.
...
- Perfis gestores (`fiscais-de-contratos` — slug canônico atual —, `fiscal-contratos` por compatibilidade, `diretores` e `teste`) cadastram e editam terceirizados em `/cadastro-terceirizados`; a rota `/requisicao-compra` permanece focada na criação, auditoria e aprovação/rejeição das requisições de compra.
...
- O saldo por item em `/requisicao-compra` parte do item da NE e abate liquidações oficiais associáveis ao subelemento, além de deduzir os valores comprometidos em requisições já enviadas ao fornecedor (`enviada_fornecedor`, `review` ou `approved`) para aquele subitem (excluindo a requisição em edição), mantendo o saldo do item coerente com o saldo disponível efetivo do empenho. Requisições em rascunho (`draft`) não reservam nem consomem saldo de outra requisição. Havendo retenção por requisições enviadas, o saldo do item exibe tooltip com o detalhamento entre saldo oficial e dedução. A origem fica persistida em `requisicao_compra_itens.source_*`; a NE do item fica em `requisicao_compra_itens.empenho_id/empenho_numero`; itens manuais continuam permitidos quando a API/cache não retorna subitens.
...
- O módulo opera com **3 status principais**: `draft` (Rascunho), `enviada_fornecedor` (Enviada ao Fornecedor) e `liquidada` (identificado na interface do usuário como **Encaminhado para pagamento**). A transição de situação na listagem é realizada diretamente pelo menu suspenso interativo **Situação** posicionado na coluna de **Ações** (ou no formulário de edição), enquanto a coluna **Situação** exibe o badge visual de status correspondente. Na tabela de requisições, todos os botões da coluna de ações permanecem organizados em linha única horizontal sem quebra (`flex-nowrap`, `whitespace-nowrap` e `shrink-0`).
- Rascunhos (`draft`) permitem edição livre sem crítica bloqueante de saldo e não abatem saldo dos empenhos.
- Ao mudar para `enviada_fornecedor`, o valor da requisição passa a ser **descontado do saldo disponível do empenho exibido no módulo de Requisição de Compra**, garantindo transparência operacional com exibição estruturada: **Saldo Oficial (SIAFI)**, **(-) Enviado ao Fornecedor** e **(=) Saldo Disponível Efetivo**. Além disso, requisições nessa situação passam a ser listadas ativamente na **Central de Notificações (`NotificationCenter`)**, com badge de status e atalho de navegação direta para `/requisicao-compra`.
- Ao mudar para `liquidada` (ação "Encaminhado para pagamento" na UI), o desconto temporário deste módulo deixa de existir, considerando que a liquidação oficial registrada no SIAFI já reduz diretamente o saldo do empenho (evitando duplicidade de desconto).
- O salvamento e transições de situação utilizam a RPC transacional `save_requisicao_compra`, que grava `requisicao_compra_empenhos`, mantém `requisicoes_compra.empenho_id/empenho_numero` preenchidos com a primeira NE por compatibilidade e valida as regras de saldo e permissões no banco.
- **Exclusão de requisições**: Gestores e fiscais podem excluir qualquer requisição diretamente na listagem (botão de lixeira na coluna Ações). Criadores (inclusive terceirizados) podem excluir suas próprias requisições enquanto estiverem em Rascunho (`draft`) ou Enviadas ao Fornecedor (`enviada_fornecedor`), sendo bloqueada a exclusão de requisições Liquidadas (`liquidada`). Ao excluir uma requisição enviada ao fornecedor, os registros vinculados (`requisicao_compra_empenhos` e `requisicao_compra_itens`) são excluídos em cascata e o saldo temporariamente comprometido no empenho é liberado imediatamente.
- Ao sair de `draft`, a requisição recebe `consumo_iniciado_em`. Seus itens passam a alimentar automaticamente o consumo de insumos em `/manutencao`, no ambiente canônico Refeitório; se retornar a rascunho, deixam de compor o consumo e a data é limpa.
...
- O módulo **Refeitório** foi posicionado logo após **Licitações** na sidebar (`appScreens.ts`, sortOrder: 36), exibido com destaque e expandido por padrão (`Layout.tsx`), contendo dois itens de acesso imediato:
  1. **Dashboard**: aponta para `/refeitorio/insumos`, renderizando o componente autônomo e reutilizável `<InsumosDashboardView defaultBloco="refeitorio" />` (posicionado acima com sortOrder: 10).
  2. **Requisição de Compra**: aponta para `/requisicao-compra` (com redirecionamento automático de `/refeitorio` e `/refeitorio/requisicoes`, posicionado com sortOrder: 20).
- Apresenta os mesmos indicadores e gráficos especializados de consumo e requisições do refeitório (KPIs de Total de Requisições e Valor Total Gasto, gráficos de Distribuição por Categoria, Valor Gasto com Insumos em barras, Consumo Geral com alternância Top 8/Todos e Top 5 Ambientes, além do modal de Detalhamento Analítico com busca e filtros).
- **Acesso e Perfis**:
  - Todo usuário que visualiza o menu **Refeitório** pode consultar a lista completa de requisições de compra do órgão e seus itens/empenhos vinculados, independentemente de quem criou a requisição. Essa ampliação é somente de leitura; criação, edição, exclusão e mudança de situação continuam sujeitas às permissões operacionais existentes.
  - Usuários terceirizados atuantes no refeitório (`tipo === 'refeitorio'`) recebem automaticamente permissões para as telas `refeitorio`, `refeitorio-insumos`, `requisicao-compra` e `empenhos`, visualizando o grupo **Refeitório** expandido com ambos os itens e o acesso à rota de **Empenhos**. Na rota `/empenhos`, o terceirizado visualiza estritamente os empenhos atribuídos a ele por vínculos diretos na tabela `terceirizado_permissions` (`empenho_id`), com o detalhamento completo (`EmpenhoDialog`) aberto em modo somente leitura (`readOnly`), sem acesso a edições ou classificações estratégicas orçamentárias.
...
`App.tsx` -> `ManutencaoAdmin.tsx` -> `manutencao.ts` -> `manutencao_ambientes` / `manutencao_ocorrencias` / `manutencao_checkins` / `manutencao_consumo_insumos` / `manutencao_blocos_mapa`

Observações:

- A interface é estruturada em 3 abas principais: `Dashboard` (painel analítico com seletor Radio entre **Avaliações** e **Insumos**, filtros por período, bloco, tipo de espaço e tipo de insumo/material no modo Insumos, KPIs executivos dedicados, gráficos analíticos e rankings executivos), `Visão Geral / Mapa` (mapa interativo do campus com editor de blocos e tabela consolidada de ambientes contendo código, nome, bloco, tipo, status, última limpeza, situação, geração de QR Code individual/em lote e exclusão) e `Ocorrências` (com listagem, fotos e resolução de chamados).
- O Dashboard conta com um seletor Radio com opções **"Avaliações"** (apresentando Satisfação Média, Total de Ocorrências, Taxa e Tempo Médio de Resolução, Gráfico de Avaliações, Pareto de Falhas e Salas com Atenção Prioritária) e **"Insumos"** (apresentando **Total de Requisições**, **Valor Total Gasto** em R$ com quantitativo consolidado de itens consumidos no período, gráfico em rosca de **Distribuição por Categoria** com participação percentual e monetária por grupo de alimentos/insumos, **Valor Gasto com Insumos** em barras, **Consumo Geral de Insumos** em barras horizontais com sanitização automática de descrições longas/códigos CATMAT e alternância Top 8/Todos, e **Top 5 Ambientes**, com filtragem dedicada e dinâmica por Tipo de Insumo).
...
- O detalhamento analítico de **Consumo de Insumos** funciona como um recurso de **Detalhamento interativo**, acionado pelo botão **Detalhar** no gráfico `Distribuição por Categoria` e por clique nos gráficos `Valor Gasto com Insumos`, `Consumo Geral de Insumos` e `Top 5 Ambientes`, abrindo modal completo com agrupamentos por ambiente/dia, busca instantânea, colunas de quantidade física e valor total financeiro em R$, e filtros por data/material. Linhas de requisição identificam número e situação e são sempre atribuídas ao Refeitório.
- Todas as tabelas de listagem (`Ocorrências` e `Visão Geral / Mapa`) contam com barra de busca e filtros rápidos na toolbar (`actions`), sem poluição de títulos redundantes ou inputs no cabeçalho das colunas. Na aba `Ocorrências`, o filtro de status é inicializado por padrão como **Pendentes**, permitindo focar imediatamente nos chamados que requerem atendimento. A tabela da aba `Visão Geral / Mapa` integra a gestão de ambientes, status operacional, histórico de última limpeza, indicadores de situação e ações de impressão individual e em lote de QR Code no layout oficial do cartaz institucional (com o QR Code renderizado perfeitamente sobre o espaço reservado no template visual) e exclusão.
...
## Extensao Comprasnet ETP

`SUAP-Atividades-Scraper-1.9/comprasnet-etp.js` -> `/comprasnet-extensao/etp` -> `comprasnetEtpService` -> `gerar-etp-comprasnet`.
...
Na versao 1.9.31, a consulta financeira do iframe passou a propagar esse cliente descartavel ate os servicos de processos, empenhos, contratos, contratos API e liquidacoes. A consulta nao depende do estado de Auth do cliente global da aplicacao; se nao terminar em 30 segundos, o iframe publica um status de erro e o content script substitui o indicador de carregamento por uma mensagem com nova tentativa.
...
Na versão 1.9.26, quando a aba ativa é `siafi.tesouro.gov.br`, o popup consulta `lc_saved_lists` pelo Supabase REST usando o JWT persistido da extensão. O RLS da tabela limita as listas ao órgão do usuário. Após a seleção, o popup identifica entre todos os frames aquele que contém a tabela `cpr-table-itens-lista`, envia a mensagem interna `siafi:fill-favorecidos` somente para esse frame e o content script acrescenta os registros ao final: preserva linhas preenchidas, usa `Incluir Favorecido` quando necessário, normaliza CPF para 11 dígitos e aplica o valor em centavos sem separador (por exemplo, `30000` para R$ 300,00), disparando `input`, `change` e `blur`. O popup divide listas maiores em blocos de no máximo 10 favorecidos e persiste o progresso por lista/transação para permitir continuar após reabrir o popup. O fluxo faz validação completa antes de alterar a página, não limpa nem sobrescreve dados existentes e nunca aciona o botão `Confirmar`; a confirmação de cada linha continua manual.

Na versão 1.9.46, a extensão injeta `siafi-predoc-alert.js` apenas no frame superior das páginas SIAFI. Em edições de DH, ela reconhece o estado do pré-doc pelos controles nativos de cada favorecido e de cada dedução visíveis na aba `Dados de Pagamento`, mantendo a pendência conhecida durante a troca de abas AJAX. O alerta de pendência só fica ativo quando essa aba está selecionada; nas demais abas, a navegação e os registros seguem livres. Se qualquer favorecido ou dedução estiver sem pré-doc enquanto `Dados de Pagamento` estiver ativa, a extensão intercepta a navegação para abas internas, links, cancelamento, `Registrar`, `Registrar Alterações` e `Salvar Rascunho`, oferecendo permanecer na página ou continuar; `Verificar Consistência` e os controles internos `Confirmar`/`Descartar` das listas de favorecidos e deduções não são bloqueados, pois são necessários para concluir a inclusão antes de abrir o pré-doc. Voltar, recarregar e fechar a guia usam o alerta nativo de saída do navegador enquanto houver pendência. O preenchimento ou a exclusão do pré-doc é observado pelo DOM, e a decisão de continuar executa uma única vez a ação original. O modal de empenho da paleta `Ctrl+K` mantém seu grid de métricas isolado dos estilos da página: quatro colunas no desktop, duas em janelas estreitas e uma no celular.

O content script global `process-paste-picker.js` (com `process-paste-picker.css`) é acionado pelo atalho `Alt + V` em qualquer página e dentro de editores/iframes (`all_frames: true`). Ele fornece um menu flutuante inteligente (estilo *Windows + V*) para colagem rápida de dados de processos abertos no SUAP. O service worker (`background.js`) rastreia abas de processos ativos (`suape_active_processes`) e ordens de ativação. Quando há múltiplos processos abertos, o menu organiza os dados agrupados por processo: primeiro todas as informações do Processo 1 (aba em foco/mais recente), depois todas as do Processo 2 e subsequentes. Apenas dados válidos identificados (Processo, SUAP ID, Beneficiário, CPF/CNPJ, Valor, NS, Contrato, Notas Fiscais, Dados Bancários, Empenhos, Retenções, Conclusão) são disponibilizados; campos vazios, nulos ou hífens isolados são estritamente omitidos. Ao confirmar com Enter ou clique, o valor é inserido no campo ativo (input, textarea ou contenteditable) e copiado para a área de transferência com feedback visual (toast).
...
O seletor de unidade usa o catálogo completo do Plano 8 (`1` a `44`) e permite uma sincronização individual da UASG ativa ou a ação `Sincronizar todas as unidades`. A ação em lote cria prévias independentes em blocos de até quatro unidades, retorna `batchId` e avança automaticamente até o lote terminar; exibe sucessos e falhas e só materializa os snapshots após a confirmação. A seleção padrão de Currais Novos continua em `DG/CN` (`19`, UASG-pai `158366`) e usa a URL histórica sem query string.
...
Na leitura do planejamento, `atividadesService` mantém somente linhas ativas (`sync_active <> false`). Para registros de `suap_plan_8`, ele também aplica `suap_unit_code` além de `campus_uasg`; isso é necessário porque Currais Novos, Jucurutu, Parelhas e o Polo de Inovação compartilham o UASG-pai `158366`. Atividades manuais continuam visíveis e o legado do Plano 8 sem unidade é tratado como Currais Novos (`19`), evitando que o valor planejado seja inflado por unidades irmãs ou versões arquivadas.
...
- Ao usar `Sincronizar todas as unidades` diretamente na aba SUAP, a extensao reutiliza a sessao `sessionid` da aba somente quando o backend solicitar reautenticacao, cria a conexão cifrada via `connect-cookie` e repete o lote; o valor nunca aparece no log.
