CREATE TABLE IF NOT EXISTS public.requisicao_compra_empenhos (
  id uuid PRIMARY KEY DEFAULT extensions.gen_random_uuid(),
  requisicao_compra_id uuid NOT NULL REFERENCES public.requisicoes_compra(id) ON DELETE CASCADE,
  empenho_id uuid NOT NULL REFERENCES public.empenhos(id) ON DELETE RESTRICT,
  empenho_numero text NOT NULL,
  sort_order integer NOT NULL DEFAULT 0,
  created_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (requisicao_compra_id, empenho_id)
);

ALTER TABLE public.requisicao_compra_itens
  ADD COLUMN IF NOT EXISTS empenho_id uuid REFERENCES public.empenhos(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS empenho_numero text;

UPDATE public.requisicao_compra_itens item
SET empenho_id = requisicao.empenho_id,
    empenho_numero = requisicao.empenho_numero
FROM public.requisicoes_compra requisicao
WHERE item.requisicao_compra_id = requisicao.id
  AND item.empenho_id IS NULL
  AND requisicao.empenho_id IS NOT NULL;
...
SELECT requisicao.id,
       requisicao.empenho_id,
       coalesce(nullif(requisicao.empenho_numero, ''), empenho.numero),
       0
FROM public.requisicoes_compra requisicao
JOIN public.empenhos empenho ON empenho.id = requisicao.empenho_id
WHERE requisicao.empenho_id IS NOT NULL
ON CONFLICT (requisicao_compra_id, empenho_id) DO UPDATE
SET empenho_numero = EXCLUDED.empenho_numero,
    sort_order = EXCLUDED.sort_order;

CREATE INDEX IF NOT EXISTS idx_requisicao_compra_empenhos_parent
  ON public.requisicao_compra_empenhos(requisicao_compra_id, sort_order);

CREATE INDEX IF NOT EXISTS idx_requisicao_compra_empenhos_empenho
  ON public.requisicao_compra_empenhos(empenho_id);

CREATE INDEX IF NOT EXISTS idx_requisicao_compra_itens_empenho
  ON public.requisicao_compra_itens(empenho_id)
  WHERE empenho_id IS NOT NULL;

ALTER TABLE public.requisicao_compra_empenhos ENABLE ROW LEVEL SECURITY;
    ...
    EXISTS (
      ...
      WHERE parent.id = requisicao_compra_empenhos.requisicao_compra_id
        AND (
          ...
          OR EXISTS (
            SELECT 1 FROM public.user_group_memberships membership
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.requisicao_compra_empenhos TO authenticated;
...
BEGIN
  ...
  IF v_status IN ('review', 'approved') THEN
    ...
    LOOP
      ...
      WHERE coalesce(item.empenho_id, requisicao.empenho_id) = (v_empenho ->> 'empenhoId')::uuid
        AND requisicao.status = 'review'
        AND (v_id IS NULL OR requisicao.id <> v_id);

      IF v_requested > greatest(0, v_balance - v_reserved) THEN
