...
describe('Descentralizacoes', () => {
  ...
  it('mostra a soma liquida inicial no card e no total da tabela', () => {
    ...
    expect(screen.getByTestId('stat-card-total-descentralizado')).toHaveTextContent('R$ 240,00');
    expect(screen.getByText('Total: R$ 240,00')).toBeInTheDocument();
  });
  ...
  it('recalcula a soma ao filtrar por dimensao, inclusive quando a dimensao vem do plano interno', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('stat-card-total-descentralizado')).toHaveTextContent('R$ 115,00');
    });

    expect(screen.getByText('Total: R$ 115,00')).toBeInTheDocument();
  ...
  it('recalcula a soma ao filtrar por origem de recurso e ao combinar origem com dimensao', async () => {
    ...
    fireEvent.change(selects[1], { target: { value: '231796' } });
    ...
    await waitFor(() => {
      expect(screen.getByTestId('stat-card-total-descentralizado')).toHaveTextContent('R$ 125,00');
    });

    expect(screen.getByText('Total: R$ 125,00')).toBeInTheDocument();

    expect(within(screen.getByRole('table')).getAllByText(/231796/)).toHaveLength(2);
  });
});
